...
function Step5({ setSelectedAddedService, setSelectedAddedServicePrice }) {
  ...
  const handleSelect = (pkg) => {
    ...
    const totalPrice = updatedSelection.reduce((acc, item) => acc + item.price, 0);
    setSelectedAddedServicePrice(totalPrice);
  };

  return (
    <div className="my-24 py-24">
          ...
          return (
            <div key={index} className="relative text-center overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="text-xl lg:text-2xl font-bold text-[#d63c3c]">${pkg.price.toFixed(2)}</div>
                <button
